...
from whispercrawler.core._types import Any, Callable, Dict, List, Optional, Tuple
from whispercrawler.core.utils import log
from whispercrawler.core.utils._shell import _CookieParser, _ParseHeaders
...
def __BuildRequest(
    ...
    # Parse parameters
    parsed_headers, parsed_cookies, parsed_params, parsed_json = __ParseExtractArguments(
        headers, cookies, params, json
    )
...
def install(force):  # pragma: no cover
    if force or not __PACKAGE_DIR__.joinpath(".whispercrawler_dependencies_installed").exists():
        ...
        __PACKAGE_DIR__.joinpath(".whispercrawler_dependencies_installed").touch()
    else:
...
def shell(code, level):
    from whispercrawler.core.shell import CustomShell
...
@option(
    "--locale", default=None, help="Specify user locale. Defaults to the system default locale."
)
@option(
