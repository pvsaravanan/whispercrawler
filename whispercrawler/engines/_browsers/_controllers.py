...
from whispercrawler.core._types import Optional, ProxyType, Unpack
from whispercrawler.core.utils import log
from whispercrawler.engines._browsers._base import AsyncSession, DynamicSessionMixin, SyncSession
from whispercrawler.engines._browsers._types import PlaywrightFetchParams, PlaywrightSession
...
class DynamicSession(SyncSession, DynamicSessionMixin):
    ...
    def fetch(self, url: str, **kwargs: Unpack[PlaywrightFetchParams]) -> Response:
        ...
        request_headers_keys = (
            {h.lower() for h in params.extra_headers.keys()} if params.extra_headers else set()
        )
        referer = (
