...
from whispercrawler.engines.static import ImpersonateType
from whispercrawler.engines.toolbelt.custom import Response as _WhisperCrawlerResponse
...
def _content_translator(
    content: Generator[str, None, None], page: _WhisperCrawlerResponse
) -> ResponseModel:
...
class WhisperCrawlerMCPServer:
    ...
    def serve(self, http: bool, host: str, port: int):
        ...
        server.add_tool(
            self.fetch, title="fetch", description=self.fetch.__doc__, structured_output=True
        )
        server.add_tool(
            self.bulk_fetch,
            title="bulk_fetch",
