...
class PaginationDetector:
    ...
    # Matched as substrings, so these must be words that don't occur incidentally.
    NEXT_TEXT_PATTERNS = [
        r"next",
        r"siguiente",
        r"proximo",
    ...
    # Symbol indicators. These are matched against the *entire* link label rather
    # than as substrings: a '>' inside prose ("C++ > Learn more") or a breadcrumb
    # ("Docs > API > Reference") is not a pagination control.
    NEXT_SYMBOL_PATTERNS = [r"»", r">", r">>", r"›", r"→"]
    ...
    def get_next_page(self) -> str | None:
        ...
        # 3b. Search by symbol labels, requiring the symbol to be the whole label
        for symbol in self.NEXT_SYMBOL_PATTERNS:
            matches = self.selector.xpath(f'//a[normalize-space(.) = "{symbol}"]')
            for match in matches:
                href = match.attrib.get("href")
                if href:
                    return urljoin(self.url, href)
        ...
        if active:
            ...
            next_sibling = active.xpath(
                "following-sibling::a[1]/@href | following-sibling::li[1]/a/@href"
            ).get()
            if next_sibling:
...
def detect_next_page(selector: Any) -> str | None:
