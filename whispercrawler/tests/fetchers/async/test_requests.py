...
from whispercrawler.fetchers import AsyncFetcher
...
class TestAsyncFetcher:
    ...
    async def test_post_properties(self, fetcher, urls):
        ...
        assert (
            await fetcher.post(urls["post_url"], data={"key": "value"}, stealthy_headers=True)
        ).status == 200
        assert (
            await fetcher.post(urls["post_url"], data={"key": "value"}, follow_redirects=True)
        ).status == 200
        assert (
    ...
    async def test_put_properties(self, fetcher, urls):
        ...
        assert (
            await fetcher.put(urls["put_url"], data={"key": "value"}, stealthy_headers=True)
        ).status in [200, 405]
        assert (
            await fetcher.put(urls["put_url"], data={"key": "value"}, follow_redirects=True)
        ).status in [200, 405]
        assert (await fetcher.put(urls["put_url"], data={"key": "value"}, timeout=None)).status in [
            200,
            405,
        ]
        assert (
