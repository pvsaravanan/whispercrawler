...
logging.getLogger("whispercrawler").setLevel(logging.DEBUG)
...
class TestElementNavigation:
    ...
    def test_ancestor_finding(self, page):
        ...
        products_with_prices = [
            price.find_ancestor(lambda p: p.has_class("product")) for price in all_prices
        ]
...
def test_large_html_parsing_performance():
    ...
    # Based on my tests with 100 runs, 1 loop each WhisperCrawler (given the extra work/features) takes 10.4ms on average
    assert (
