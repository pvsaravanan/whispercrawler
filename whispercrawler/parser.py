...
from whispercrawler.core._types import (
    TYPE_CHECKING,
    Any,
    Callable,
    Dict,
...
class Selector(SelectorsGeneration):
    ...
    def __init__(
        ...
        **_,
        ...
        if root is None:
            ...
            self._root = cast(
                HtmlElement, fromstring(body or "<html/>", parser=parser, base_url=url or "")
            )
    ...
    def css_first(self, selector: str, default: Any = None) -> Optional["Selector"]:
        """Return the first element that matches the CSS selector, or default if none match."""
        results = self.css(selector)
        return results[0] if results else default

    def xpath_first(self, selector: str, default: Any = None) -> Optional["Selector"]:
        """Return the first element that matches the XPath selector, or default if none match."""
    ...
    def find_similar(
        ...
        target_attrs = (
            self.__get_attributes(root, ignore_attributes) if ignore_attributes else root.attrib
        )
...
class Selectors(List[Selector]):
    ...
    def xpath(
        ...
        **Important:
        ...
        """
        results = [
            n.xpath(selector, identifier or selector, False, auto_save, percentage, **kwargs)
            for n in self
        ]
