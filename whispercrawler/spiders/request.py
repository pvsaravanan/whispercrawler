...
class Request:
    def __init__(
        ...
        url: str,
        sid: str = "",
        callback: Callable[[Response], AsyncGenerator[Union[Dict[str, Any], "Request", None], None]]
        | None = None,
        priority: int = 0,
    ...
    def update_fingerprint(
        ...
        if include_kwargs:
            kwargs = (
                key.lower()
                for key in self._session_kwargs.keys()
        ...
        if include_headers:
            ...
            for key, value in headers.items():
                processed_headers[_convert_to_bytes(key.lower()).hex()] = _convert_to_bytes(
                    value.lower()
                ).hex()
    ...
    def __getstate__(self) -> dict[str, Any]:
        ...
        state["_callback_name"] = (
            getattr(self.callback, "__name__", None) if self.callback is not None else None
        )
