...
When scraping at scale, you'll often need to rotate through multiple proxies to avoid rate limits and blocks. WhisperCrawler's `ProxyRotator` makes this straightforward — it works with all session types and integrates with the spider's blocked request retry system.
...
```python
from whispercrawler.fetchers import AsyncDynamicSession, AsyncStealthySession, ProxyRotator
...
```python
from whispercrawler.core._types import ProxyType
...
```python
from whispercrawler.spiders import Spider, SessionManager, Request, Response
...
```python
from whispercrawler.spiders import Spider, SessionManager, Request, Response
from whispercrawler.fetchers import FetcherSession, AsyncStealthySession, ProxyRotator
