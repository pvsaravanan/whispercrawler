...
To ensure it's working, use the full path to the `whispercrawler` executable. Open the terminal and execute the following command:

1. **MacOS**: `which whispercrawler`
2. **Windows**: `where whispercrawler`

For me, on my Mac, it returned `/Users/<MyUsername>/.venv/bin/whispercrawler`, so the config I used in the end is:
```json
{
  "mcpServers": {
    "WhisperCrawlerServer": {
      "command": "/Users/<MyUsername>/.venv/bin/whispercrawler",
      "args": [
        "mcp"
      ]
    }
...
Then, after you've added the server, you need to completely quit and restart the app you used above. In Claude Desktop, you should see an MCP server indicator (🔧) in the bottom-right corner of the chat input or see `WhisperCrawlerServer` in the `Search and tools` dropdown in the chat input box.
...
```bash
whispercrawler mcp --http --host '127.0.0.1' --port 8000
```
