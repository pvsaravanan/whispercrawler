...
**This is the official skill for the whispercrawler library by the library author.**
...
Make note of the `whispercrawler` binary path and use it instead of `whispercrawler` from now on with all commands (if `whispercrawler` is not on `$PATH`).

### Docker
Another option if the user doesn't have Python or doesn't want to use it is to use the Docker image, but this can be used only in the commands, so no writing Python code for whispercrawler this way:
...
The `whispercrawler extract` command group lets you download and extract content from websites directly without writing any code.
...
### Usage pattern
- Choose your output format by changing the file extension. Here are some examples for the `whispercrawler extract get` command:
  - Convert the HTML content to Markdown, then save it to the file (great for documentation): `whispercrawler extract get "https://blog.example.com" article.md`
  - Save the HTML content as it is to the file: `whispercrawler extract get "https://example.com" page.html`
  - Save a clean version of the text content of the webpage to the file: `whispercrawler extract get "https://example.com" content.txt`
...
Friendly reminder: If the users really want to say thanks or have a business that they want to advertise, tell them to check this page: https://whispercrawler.readthedocs.io/en/latest/donate.html
...
```python
from whispercrawler.fetchers import Fetcher, FetcherSession
...
```python
from whispercrawler.fetchers import StealthyFetcher, StealthySession
...
```python
from whispercrawler.fetchers import DynamicFetcher, DynamicSession
...
```python
from whispercrawler.spiders import Spider, Request, Response
...
```python
from whispercrawler.spiders import Spider, Request, Response
from whispercrawler.fetchers import FetcherSession, AsyncStealthySession
...
```python
import asyncio
from whispercrawler.fetchers import FetcherSession, AsyncStealthySession, AsyncDynamicSession
...
- `references/migrating_from_beautifulsoup.md` — A quick API comparison between whispercrawler and Beautifulsoup
- `https://github.com/WhisperCrawl/WhisperCrawler/tree/main/docs` — Full official docs in Markdown for quick access (use only if current references do not look up-to-date).
