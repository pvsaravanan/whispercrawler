...
docker run -i --rm whispercrawler mcp
```

The MCP server name when registering with a client is `WhisperCrawlerServer`. The command is the path to the `whispercrawler` binary and the argument is `mcp`.
